...
class TurbidityCurrent2D(Component):
    ...
    def __init__(
        ...
        **kwds
        ...
        nu: float, optional
            Kinematic viscosity of water(at 293K)(m^2/s)
        ...
        p_val: float, optional
            sediment entrainment coeficient for field scale in Traer2012 and GP1991field
        no_erosion: boolean, optional
            If True, bed cannot be eroded
        no_erosion_Ds: ndarray, optional
            If no_erosion is True, set the target grain sizes by the order
        ...
        if config_path is None:
            ...
            self.no_erosion_Ds = no_erosion_Ds
            self.dt_local_limit = dt_local_limit
            self.alpha_TKE = alpha_TKE
            ...
            self.ewc = config['flow']['ewc']
            self.water_detrainment = config['flow']['water_detrainment']
            self.det_factor = config['flow']['det_factor']
            self.suspension = config['flow']['suspension']
            self.bedload_transport = config['flow']['bedload_transport']
            self.sed_entrainment_func = config['flow']['sed_entrainment_func']
            self.p_coef = config['flow']['p_coef']
            self.camax = config['flow']['camax']
            self.model = config['flow']['model']
            self.karman = config['flow']['karman']
            self.no_erosion = config['flow']['no_erosion']
            self.no_erosion_Ds = config['flow']['no_erosion_Ds']
            self.dt_local_limit = config['flow']['dt_local_limit']
            self.der_active_layer = 0
            self.salt = config['flow']['salt']
            self.alpha_TKE = config['flow']['alpha_TKE']
            self.duration = config['flow']['duration']
        ...
        try:
            ...
            for i in range(self.number_gclass):
                self.max_erosion_i[i, :] = grid.at_node[
                    "max_erosion_" + str(i)
                ]
    ...
    def run_one_step(self, dt=None):
        ...
        for i in range(self.number_gclass):
            #self.init_C[i,:] = self.init_C[i,:] * 0 + 0.0001
            #self.C_i[i,:] = self.C_i[i,:] + self.init_C[i,:]
            self.C_i[i,
                     :] = self.grid["node"]["flow__sediment_concentration_" + str(i)]
            self.Ch_i[i, :] = self.C_i[i,:]  * self.h #+ self.init_Ch[i,:]
        self.C[:] = np.sum(self.C_i, axis=0)
        self.Ch[:] = np.sum(self.Ch_i, axis=0)
        #print(self.C_i)
    ...
    def _friction(self):
        ...
        if self.water_entrainment:
            ...
            self.ew_node[self.wet_nodes] = get_ew(
                self.U_node[self.wet_nodes], self.Ch[self.wet_nodes], self.R, self.g, self.ewc,
            )
        else:
        ...
        if self.water_detrainment:
            # Fr_horizontal = U_node[nodes] / np.sqrt(self.g * (h[nodes]+0.01))
    ...
    def _remove_abnormal_values(self):
        ...
        """
        if len(self.wet_pwet_nodes[self.h_temp[self.wet_pwet_nodes] < 0]) > 0:
        ...
        for i in range(self.number_gclass):
            if len(self.wet_pwet_nodes[self.Ch_i_temp[i, self.wet_pwet_nodes] < 0]) > 0:
                adjust_negative_values(
                    self.Ch_i_temp[i, :],
                    self.wet_pwet_nodes,
                    self.node_east,
    ...
    def calc_deposition(
        ...
        # U_Ch = U_node[nodes]**2 / self.Ch_i_prev[:, nodes]
        # Ri = self.R * self.g * self.Ch_i_prev[:, nodes] / U_node[nodes] ** 2
        # slope_inside = U_Ch * self.Cf[nodes] /Ri
        # slope_inside = U_node[nodes]**2 / (self.Cf_node[nodes] * h[nodes])
        ...
        #elev = self._grid.at_node["topographic__elevation"][nodes]
        if self.no_erosion is True:
            nds = self.no_erosion_Ds
            if nds is None:
                ...
                for i in nds:
                    eroded_region, threshold_elev, uneroded_region = [],[],[]
                    phi = -np.log2(self.Ds[i]*1000)
                    phi_depth = 170 * phi + 380 #Amami emprical regression
                    threshold_elev = self._grid.at_node["topographic__elevation"][nodes] < -phi_depth
                    eroded_region = self.bed_change_i[i, nodes] < 0.0
                    uneroded_region = eroded_region * threshold_elev
                    out_Ch_i[i, nodes[uneroded_region]
                            ] = self.Ch_i_prev[i, nodes[uneroded_region]]
                    self.bed_change_i[i, nodes[uneroded_region]] = 0.0
        ...
        # if no_erosion is false, The eroded volume of sediment is fed to the active layer.
        # if self.no_erosion is False:
        #     if self.salt == False:
        #         for i in range(self.number_gclass):
        #             eroded_region = np.sum(self.bed_change_i[i, nodes], axis=0) < 0.0
        #             self.bed_active_layer[i, nodes[eroded_region]] += self.bed_change_i[i, nodes[eroded_region]]/self.number_gclass
        #     elif self.salt == True:
        #         for i in range(self.number_gclass - 1):
        #             eroded_region = np.sum(self.bed_change_i[i, nodes], axis=0) < 0.0
        #             self.bed_active_layer[i, nodes[eroded_region]] += self.bed_change_i[i, nodes[eroded_region]]/(self.number_gclass - 1)
    ...
    def _bed_diffusion_at_high_slope(self):
        """diffusion sediment transport in the region where slope is close to angle
             of repose
        """
        diffusion_coeff = 1.0e-3 #def 1.0e-10
        high_slope = 0.6 #1.0 #45 degree
    ...
    def save_grid(self, filename, clobber=True):
        ...
        # save_grid(self.es, filename, clobber=clobber)
    ...
    def save_nc(self, filename, time=None, format="NETCDF3_64BIT"):
        ...
        # lats = np.linspace(ylim[0], ylim[1], self.grid.number_of_nodes)
        # lons = np.linspace(xlim[0], xlim[1], self.grid.number_of_nodes)
        # self.grid.at_node["y"] = lats
        # self.grid.at_node["x"] = lons
    ...
    def save_point_data(self, filename, obs_cordiate, i):

         #142401, 729026
        distances = np.sqrt((self.grid.node_x - obs_cordiate[0])**2 + (self.grid.node_y - obs_cordiate[1])**2)
        node_index = np.argmin(distances)

        # x_size = (xlim[1] - xlim[0]) / grid_degree
        # y_size = (ylim[1] - ylim[0]) / grid_degree

        out = [
                np.sqrt(self.grid.at_node['flow__vertical_velocity_at_node'][node_index]**2 \
                + self.grid.at_node["flow__horizontal_velocity_at_node"][node_index]**2), \
                self.grid.at_node["flow__depth"][node_index], \
                self.ew_node[node_index], \
                # self.Cf_node[node_index], \
                #self.grid.at_node["flow__sediment_concentration_total"][node_index], \
        ...
        with open(filename, 'a') as f:
            writer = csv.writer(f)
            if i==1:  # Check if header has been written
