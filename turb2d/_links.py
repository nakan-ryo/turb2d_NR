...
if numpy_version >= '1.24.0':
    dtype = np.int32  # NumPy 1.20以降では、`int`を推奨
else:
    dtype = np.int  # 古いバージョンでは`np.int32`が使用されることが多い
def neighbors_at_link(shape, links):
    ...
    # バージョン1.20以降かどうかをチェック


    # 配列を指定のdtypeで作成
    links = np.asarray(links, dtype=dtype)
    out = np.full((links.size, 4), -1, dtype=dtype)
...
def vertical_active_link_ids(shape, active_ids, bad_index_value=-1):
    ...
    out = np.full(number_of_vertical_links, bad_index_value, dtype=dtype)
