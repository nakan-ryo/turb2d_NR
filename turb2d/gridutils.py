...
def adjust_negative_values(
    f, core, east_id, west_id, north_id, south_id, out_f=None, loop=1,
):
    ...
    while len(to_fix) > 0 and counter < loop:
        ...
        to_fix = core[out_f[core] < 0]
        f_temp[:] = out_f[:]
        # print(counter, np.min(out_f))

        # if np.min(out_f) < 0:  
        #     a = np.min(out_f)
        #     with open('/mnt/d/turb2d/filter_nu05.csv', 'a') as f:
        #         writer = csv.writer(f)
        #         writer.writerow([a])     

    if counter == loop:
        # print("failed", np.min(out_f))
        out_f[out_f < 0] = 0
